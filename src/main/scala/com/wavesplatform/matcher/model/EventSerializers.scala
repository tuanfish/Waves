...
import com.wavesplatform.transaction.assets.exchange.{AssetPair, Order, OrderJson}
import kamon.Kamon
...
class EventSerializers extends SerializerWithStringManifest {
  ...
  override def toBinary(o: AnyRef): Array[Byte] = o match {
    ...
    case oa: OrderAdded                     => encodeAndMeasure("order-added", encodeOrder(oa.order))
  ...
  override def fromBinary(bytes: Array[Byte], manifest: String): AnyRef = manifest match {
    ...
    case Manifest.OrderAddedV2     => decodeOrder(bytes)
...
object EventSerializers {
  ...
  private def encodeOrder(o: LimitOrder) = {
    val orderBytes = 1.toByte +: o.order.bytes() // todo: change this to support orders v2+
    ByteBuffer
  ...
  private def decodeOrder(bytes: Array[Byte]) = {
    ...
    val order = orderBytes.head match {
      // todo: support orders v2+
      case 1 => Order.parseBytes(orderBytes.tail).get
    }
